...
export class AttendanceAnalyticsRepository {
  ...
  // ─── Leave Analytics Merged Methods ───────────────────────────────────────
  async getExecutiveKPIs() {
    ...
    const [reqsRes, leaveTodayRes, deptTopRes, costRes, encashRes] = await Promise.all([
      ...
      dbService.query(
        `SELECT SUM(amount_paid) as total_encashment_payout, COUNT(*) as encashment_count
  ...
  async logLeaveReportExport(actorId: number, reportType: string, filename: string) {
    try {
      await dbService.query(
        `INSERT INTO audit_logs (employee_id, action, module, details)
