...
export const EnterpriseLeavePage: React.FC<{ onNavigate?: (tab: string) => void }> = ({ onNavigate }) => {
  ...
  const [tab, setTab] = useState<'balances' | 'applications' | 'ledger' | 'analytics'>('balances');
  ...
  useEffect(() => { 
    if (tab === 'analytics') fetchLeaveAnalytics();
    else fetchData(); 
  }, [fetchData, fetchLeaveAnalytics, tab]);
    ...
    <div className="space-y-5 min-h-screen pb-10 font-sans text-slate-800">
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1.5 border border-slate-200 overflow-x-auto">
        ...
        {isManager && (
          <button onClick={() => setTab('analytics')}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
              tab === 'analytics' ? 'bg-white text-emerald-700 shadow-sm border border-emerald-100' : 'text-slate-500 hover:text-slate-800'
            }`}>
            <BarChart2 className="w-4 h-4" /> Leave Analytics
          </button>
        )}
      </div>
      ...
      {tab === 'analytics' && isManager && (
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="text-center py-12"><Activity className="w-8 h-8 text-emerald-500 animate-spin mx-auto" /><p className="text-slate-500 mt-2">Loading Analytics...</p></div>
          ) : (
            <>
              {leaveKpi && (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm text-center">
                    ...
                    <h4 className="text-xl font-bold text-emerald-600">{Number(leaveKpi.avg_duration || 0).toFixed(1)}d</h4>
                  </div>
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm text-center">
                  ...
                  <div className="h-64 flex items-end gap-2 px-2">
                    {leaveTrend.map((t, i) => (
                      <div key={i} className="flex-1 flex flex-col justify-end group relative h-full">
                        <div className="w-full bg-emerald-500 rounded-t-sm" style={{ height: `${Math.max(5, (t.count / Math.max(...leaveTrend.map(x => x.count), 1)) * 100)}%` }}></div>
                    ...
                    {leaveDepts.map((d, i) => (
                      <div key={i} className="flex flex-col gap-1 text-sm">
                        ...
                        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full bg-emerald-400" style={{ width: `${Math.min(100, (d.request_count / Math.max(...leaveDepts.map(x => x.request_count), 1)) * 100)}%` }}></div>
                        </div>
                      </div>
                    ))}
